...
login: (
  ...
  linkGoogleIdentity: () => Promise<{
    success: boolean;
    message?: string;
  }>;
  unlinkGoogleIdentity: () => Promise<{
  success: boolean;
  message?: string;
}>;
  register: (
...
const getStoredLastActivityAt = useCallback((role?: User['role']) => {
  const storage = getActivityStorage(role);
  const raw = storage.getItem(LAST_ACTIVITY_AT_KEY);
  const parsed = raw ? Number(raw) : 0;
  return Number.isFinite(parsed) && parsed > 0 ? parsed : Date.now();
}, [getActivityStorage]);

  const writeActivity = useCallback(
  (role?: User['role'], timestamp = Date.now()) => {
    const storage = getActivityStorage(role);
    storage.setItem(LAST_ACTIVITY_AT_KEY, String(timestamp));
  ...
  const getAuthNotice = useCallback((): AuthNotice | null => {
...
const hasOAuthCollisionNotice = useCallback(() => {
  const notice = getAuthNotice();
  return notice?.code === 'oauth_same_email_existing_account';
}, [getAuthNotice]);
...
const claimBrowserSession = useCallback(
  ...
  writeActivity(userRef.current?.role, Date.now());
  setShowSessionWarning(false);
  setSessionCountdown(0);
    ...
    try {
      ...
      if (!data) {
        ...
        if (existingByEmailError) {
          console.error(
            'Failed to check existing profile by email:',
        ...
        if (existingByEmail) {
          // Same email exists, but belongs to a different auth user.
          // Do NOT rewrite users.user_id here.
          if (existingByEmail.user_id !== authUser.id) {
            ...
            console.warn(
              'OAuth/email collision detected. Existing profile belongs to another auth user.',
              {
                existingUserId: existingByEmail.user_id,
                incomingAuthUserId: authUser.id,
                email,
                provider,
              }
            );
...
if (!profile) {
  const hasCollision = hasOAuthCollisionNotice();
  ...
  if (hasCollision) {
    showIndicator(
      'This email already belongs to an existing account. Sign in with your original method first.',
      'security'
    );
  }
...
const profile = await fetchOrCreateUserProfile(data.user);
  ...
  return {
    success: false,
    error: hasCollision ? 'oauth_email_conflict' : 'account_inactive',
  };
}
...
const linkGoogleIdentity = useCallback(async () => {
  if (authActionPending) {
    return { success: false, message: 'Another auth action is running.' };
  }
  ...
  try {
    ...
    return {
      success: true,
      message: 'Redirecting to Google to link account...',
    };
  } catch (err) {
...
const unlinkGoogleIdentity = useCallback(async () => {
  ...
  try {
    ...
    if (!googleIdentity) {
      return {
        success: false,
        message: 'No linked Google account was found.',
      };
    }
...
const storage = user.role === 'admin' ? localStorage : sessionStorage;

if (!storage.getItem(LAST_ACTIVITY_AT_KEY)) {
  writeActivity(user.role, Date.now());
}

    const evaluateSessionState = () => {
      const lastActivityAt = getStoredLastActivityAt(user.role);
      const now = Date.now();
