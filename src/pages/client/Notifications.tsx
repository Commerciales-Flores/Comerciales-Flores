...
const NotificationCard = React.memo(function NotificationCard({
  ...
  return (
    <motion.div
      ...
      className={`relative flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-all sm:gap-4 sm:p-5 ${
        isSelected
    ...
    >
      {selectionMode && (
        <div className="pt-1">
          <div
            className={`flex size-5 items-center justify-center rounded-md border-2 transition-colors sm:size-6 ${
              isSelected
          ...
          >
            ...
            <div className="bg-white p-6">
              <button
                type="button"
                onClick={onClose}
                className="min-h-[44px] w-full rounded-2xl bg-blue-600 py-3.5 font-semibold text-white transition hover:bg-blue-700 active:scale-95"
              >
                Close Notification
              </button>
            </div>
...
export default function ClientNotifications() {
  ...
  const [selectedNotifs, setSelectedNotifs] = useState<Set<string>>(
    () => new Set()
  );
  const [quickViewNotif, setQuickViewNotif] = useState<Notification | null>(
    null
  );
  ...
  const totalPages = useMemo(() => {
    return Math.max(1, Math.ceil(filteredNotifications.length / ITEMS_PER_PAGE));
  ...
  const handleFilterSelectFromSheet = useCallback(
  ...
  {!selectionMode && hasNotifications && (
    <>
      <button
        ...
        className="flex min-h-[40px] items-center gap-2 rounded-lg bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-600 transition hover:bg-blue-100"
      >
      ...
      <button
        ...
        className="min-h-[40px] rounded-lg bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-600 transition hover:bg-blue-100"
      >
        Mark all as read
      </button>
    </>
              ...
              <EmptyState
                ...
                description={
                  searchQuery.trim()
                    ? `No ${
                        filter === 'all' ? '' : `${filter} `
                      }notifications matched "${searchQuery.trim()}".`
                    : 'Try changing your filters to see more updates.'
                }
              />
              ...
              <div className="mt-4 flex justify-center gap-2">
                {searchQuery.trim() && (
                  <button
                    ...
                    className="min-h-[40px] rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
                  >
                    Clear Search
                  </button>
                )}
              ...
              ({ group, items }) =>
                items.length > 0 && (
                  <div key={group} className="space-y-4">
                    <h4 className="px-1 text-xs font-bold uppercase tracking-widest text-gray-400 sm:text-sm">
                      {group}
                    </h4>
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs font-bold transition hover:bg-white/20 sm:text-sm"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-red-500 px-3 py-2 text-xs font-bold transition hover:bg-red-600 sm:text-sm"
                >
