...
export default function ClientMessages() {
  ...
  // ✅ Local state for live database fetching
  ...
  // ✅ Fetch Messages directly from Supabase on mount
  useEffect(() => {
  ...
  const handleNewInquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Await the Supabase insertion and capture the new ID
      const newInquiryId = await addInquiry({
        userId: user.id,
        name: `${user.firstName} ${user.lastName}`, // Fixed user name mapping
        email: user.email,
        subject: newInquiryForm.subject,
        message: newInquiryForm.message,
      });

      // Update local UI immediately so the user sees it without refreshing
              ...
              <div className="bg-white rounded-lg border border-gray-200 h-full flex flex-col">
                <div className="p-6 border-b border-gray-200">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={`px-3 py-1 text-sm font-medium rounded-full ${statusColors[selectedInquiry.status]}`}>
                ...
                {selectedInquiry.status === 'resolved' && (
                    <div className="p-4 border-t border-gray-200 flex items-center justify-center gap-2 text-green-700 bg-green-50 font-medium">
