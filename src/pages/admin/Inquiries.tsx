...
export default function AdminInquiries() {
  ...
  useEffect(() => {
    if (selectedInquiry || sortedInquiries.length === 0) return;
    if (window.innerWidth >= 1024) {
      setSelectedInquiry(sortedInquiries[0].id);
    }
  }, [selectedInquiry, sortedInquiries]);
  ...
  const handleRespond = useCallback(() => {
    ...
    if (currentInquiry?.userId) {
      sendInquiryResponseNotification(currentInquiry.userId, currentInquiry.subject);
    }

    setResponse('');
  }, [selectedInquiry, response, inquiries, updateInquiry, sendInquiryResponseNotification]);
  ...
  return (
    <div className="max-w-6xl mx-auto py-4 md:py-8 px-4 space-y-6 h-[calc(100vh-80px)] md:h-[calc(100vh-120px)] flex flex-col">
      <div
        ...
        } flex-col md:flex-row md:items-center justify-between gap-4 shrink-0`}
      >
        <div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0 relative">
        <div
          ...
          } lg:w-1/3 flex-col gap-3 overflow-y-auto pr-2 custom-scrollbar`}
        >
