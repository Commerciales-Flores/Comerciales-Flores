...
const [unlinkingGoogle, setUnlinkingGoogle] = useState(false);
  ...
  const fullName = useMemo(() => {
    return `${user?.firstName ?? ''} ${user?.lastName ?? ''}`.trim() || (isAdmin ? 'Administrator' : 'User');
  ...
  try {
    ...
    if (!result?.success) {
      showMessage('error', result?.message || 'Failed to link Google account.');
      return;
    }

    setIsGoogleUser(true);
    showMessage('success', result.message || 'Google account linked successfully.');
  } catch {
    showMessage('error', 'Failed to link Google account.');
  } finally {
    setLinkingGoogle(false);
  }
}, [linkGoogleIdentity, linkingGoogle, unlinkingGoogle, showMessage]);
...
const handleUnlinkGoogle = useCallback(async () => {
  ...
  if (!hasPasswordIdentity) {
    showMessage(
      'error',
      'Set a password first before unlinking your Google account.'
    );
    return;
  }
  ...
  try {
    ...
    if (!result?.success) {
      showMessage('error', result?.message || 'Failed to unlink Google account.');
      return;
    }

    setIsGoogleUser(false);
    showMessage('success', result.message || 'Google account unlinked successfully.');
  } catch {
    showMessage('error', 'Failed to unlink Google account.');
  } finally {
    setUnlinkingGoogle(false);
  }
}, [hasPasswordIdentity, linkGoogleIdentity, unlinkGoogleIdentity, linkingGoogle, unlinkingGoogle, showMessage]);
  ...
  <button
    ...
    disabled={savingProfile}
    className="inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-50 sm:w-auto"
  >
    <X className="size-4" />
    Cancel
  </button>
              ...
              <div className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                                  Update your password and secure administrative access.
                    </p>
                  </div>
                </div>
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          showMessage(
                                            ...
                                            result.message ||
                                              (result.success
                                                ? 'Redirecting to Google to link your account...'
                                                : 'Unable to link Google account.')
                                          );
                                        }}
                                        className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-700 transition hover:bg-blue-100"
                                      >
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          if (!hasPasswordIdentity) {
                                            showMessage(
                                              'error',
                                              'Add a password first before unlinking Google.'
                                            );
                                            return;
                                          }
                                        ...
                                        className={`inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${
                                          hasPasswordIdentity
                                            ? 'border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100'
