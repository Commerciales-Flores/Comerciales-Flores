                ...
                userReservationsWithBalance.length > 0 ? (
                  userReservationsWithBalance.map(reservation => (
                    <div key={reservation.id} onClick={() => { setSelectedTargetId(reservation.id); setStage('fill_form'); }} className="p-3 hover:bg-blue-50 cursor-pointer transition-colors">
                      ...
                      <p className="text-sm text-red-600">Balance: {formatCurrency(reservation.totalAmount - reservation.paidAmount)}</p>
                      <p className="text-xs text-gray-500">ID: {reservation.id}</p>
                    </div>
                  ))
                ) : (
                  <div className="p-8 text-center text-gray-500">This user has no reservations with an outstanding balance.</div>
                )
              )}
