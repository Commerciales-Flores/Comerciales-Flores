...
import { adminTicketReplyTemplate } from "../_shared/templates.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const secret = Deno.env.get("ADMIN_TICKET_REPLY_SECRET");

    if (!secret) {
      return jsonResponse(
        { success: false, reason: "Missing ADMIN_TICKET_REPLY_SECRET." },
        500
      );
    }
    ...
    if (!payload?.ticketId || !payload?.replyMessage?.trim()) {
      return jsonResponse(
        {
          success: false,
          reason: "ticketId and replyMessage are required.",
        },
        400
      );
    }
    ...
    for (const recipient of (admins ?? []) as AdminRow[]) {
      const emailTemplate = adminTicketReplyTemplate({
