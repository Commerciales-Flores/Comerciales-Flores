...
import { adminNewTicketTemplate } from "../_shared/email/templates.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const secret = Deno.env.get("ADMIN_NEW_TICKET_SECRET");

    if (!secret) {
      return jsonResponse(
        { success: false, reason: "Missing ADMIN_NEW_TICKET_SECRET." },
        500
      );
    }
    ...
    for (const recipient of (admins ?? []) as AdminRow[]) {
      const emailTemplate = adminNewTicketTemplate({
